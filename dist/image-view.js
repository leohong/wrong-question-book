...
export function photoAttributes(value,small=false){
...
export function observeImages(root=document.body){
  ...
  function pump(){while(!stopped&&active.size<4&&pending.length){const img=pending.shift();if(!img.isConnected||!tracked.has(img))continue;active.add(img);getImage(img.dataset.photo,img.dataset.small==='true').then(blob=>{
  ...
  const changes=new MutationObserver(scan);changes.observe(root,{childList:true,subtree:true});scan();
