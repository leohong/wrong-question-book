...
export function validateBackup(data, {allowImageRefs=false}={}) {
  ...
  return {version:1,categories:[...data.categories],target:data.target,cards:data.cards.map(c=>({id:c.id,title:c.title,category:c.category,question:c.question,answer:c.answer,streak:c.streak,stage:c.stage,attempts:c.attempts,mistakes:c.mistakes,created:c.created,due:c.due})),history:data.history.map(h=>({cardId:h.cardId,category:h.category,correct:h.correct,at:h.at}))};
}
