...
export function eligible(cards, category, mode, now=Date.now()) {
  return cards.filter(c=>c.answer && (!category || c.category===category) && (mode==='all' || (mode==='due' ? c.stage>=0 && c.due<=now : c.stage<0 || c.due<=now)));
}
...
export function validateBackup(data, {allowImageRefs=false}={}) {
  ...
  return {version:1,categories:[...data.categories],target:data.target,cards:data.cards.map(c=>({id:c.id,title:c.title,category:c.category,question:c.question,answer:c.answer,...(c.questionMask?{questionMask:structuredClone(c.questionMask)}:{}),...(c.answerMask?{answerMask:structuredClone(c.answerMask)}:{}),...(c.questionOriginal?{questionOriginal:c.questionOriginal}:{}),streak:c.streak,stage:c.stage,attempts:c.attempts,mistakes:c.mistakes,created:c.created,due:c.due})),history:data.history.map(h=>({cardId:h.cardId,category:h.category,correct:h.correct,at:h.at}))};
}
