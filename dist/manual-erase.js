export function manualErase(source, dialog, onNext, onBack, initialMask=null) {
  ...
  function show(){
    ...
    function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(source,0,0);for(const s of strokes)drawStroke(s);if(active?.stroke)drawStroke(active.stroke);$('erase-undo').disabled=$('erase-reset').disabled=!strokes.length;$('erase-status').textContent=`已抹除 ${strokes.length} 筆。${pan?'目前可拖曳移動圖片。':'目前為橡皮擦模式。'}`;}
    ...
    canvas.onpointerdown=e=>{if(active||e.button!==0)return;e.preventDefault();canvas.setPointerCapture(e.pointerId);active={id:e.pointerId,x:e.clientX,y:e.clientY,left:viewport.scrollLeft,top:viewport.scrollTop};if(!pan)active.stroke={size:Number($('erase-size').value)*canvas.width/canvas.getBoundingClientRect().width,points:[point(e)]};draw();};
    canvas.onpointermove=e=>{if(!active||active.id!==e.pointerId)return;if(active.stroke){active.stroke.points.push(point(e));draw();}else{viewport.scrollLeft=active.left+active.x-e.clientX;viewport.scrollTop=active.top+active.y-e.clientY;}};
    function finish(e,cancel=false){if(!active||active.id!==e.pointerId)return;if(!cancel&&active.stroke)strokes.push(active.stroke);active=null;if(canvas.hasPointerCapture(e.pointerId))canvas.releasePointerCapture(e.pointerId);draw();}
