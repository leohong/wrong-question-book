export function manualErase(source, dialog, onNext, onBack, initialMask=null) {
  ...
  function show(){
    ...
    canvas.onpointerdown=e=>{if(active||e.button!==0)return;e.preventDefault();canvas.setPointerCapture(e.pointerId);active={id:e.pointerId,x:e.clientX,y:e.clientY,left:viewport.scrollLeft,top:viewport.scrollTop};if(!pan){const p=point(e),rgb=ctx.getImageData(Math.min(canvas.width-1,Math.floor(p.x)),Math.min(canvas.height-1,Math.floor(p.y)),1,1).data;picked='#'+Array.from(rgb.slice(0,3),v=>v.toString(16).padStart(2,'0')).join('');active.stroke={color:picked,size:Number($('erase-size').value)*canvas.width/canvas.getBoundingClientRect().width,points:[p]};}draw();};
