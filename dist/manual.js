...
export const MANUAL_VERSION='2026-09-17.5';
...
<section class="panel"><h2>1. 快速新增題目</h2><ol><li>在「我的題庫」按「快速新增」，選擇分類後拍照或選圖。</li><li>拖曳框選一道題目；可移動方框、拉動四邊或保留全圖。</li><li>答案選擇「使用題目原圖」、「另拍答案」、「選擇答案圖片」、「文字答案」或「稍後補答案」。</li><li>按「儲存並結束」，或按「儲存並繼續」立即加入下一題；連續新增時也可按「存檔結束」。</li></ol><p>每次按下儲存後，卡片就會立即寫入題庫。之後按取消只會放棄目前尚未儲存的題目，不會刪除前面已完成的卡片。</p><p>快速模式會記住上一次分類、自動產生題目名稱，並以平衡畫質儲存。它會略過抹除和清晰度確認，以縮短收藏時間。</p></section>
...
<section class="panel"><h2>6. 自動產生考卷</h2><p>在「產生考卷」設定卷名、分類、出題範圍和題數。系統會從符合條件的卡片隨機抽題，同一份考卷不會重複；沒有答案的卡片也能加入，答案卷會改為顯示原題。</p><p>考卷固定採 A4 直式雙欄版面，不會因手機方向改成單欄。每欄最多 5 題、每頁最多 10 題，題目之間使用虛線分隔；可在預覽上雙指縮放，或使用縮放列調整大小及符合螢幕。</p><p>可選擇是否附上答案卷，也能重新抽題。完成預覽後按「列印／另存 PDF」；在瀏覽器列印視窗選擇印表機或「另存為 PDF」。產生考卷不會改變熟練度。</p></section>
