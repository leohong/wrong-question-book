...
export const MANUAL_VERSION='2026-09-15.3';
...
<section class="panel"><span class="tag">本次更新</span><h2>更快收藏，也能保留完整內容</h2><ul><li>新增「快速新增」三步驟流程，可連續拍攝多題。</li><li>題目與答案文字支援 Markdown 和數學公式。</li><li>圖片與 AI 指令可分開複製，手機可嘗試同時分享圖片與文字。</li></ul></section>
...
<section class="panel"><h2>4. Markdown 與數學公式</h2><p>題目與答案文字支援標題、粗體、斜體、清單、引用、程式碼與網址。行內公式用 <code>$...$</code>，例如 <code>$-\\frac{19}{7}$</code>；獨立公式用 <code>$$...$$</code>。</p><p>完整編輯時會即時預覽，題庫卡片、題目詳情及練習畫面也會顯示格式。每個欄位最多 10,000 字；無法解析的公式會保留原文。</p></section>
<section class="panel"><h2>5. 搭配 Gemini 或 ChatGPT 辨識</h2><p>在圖片卡片展開 AI 工具後，可分別按「複製圖片」及「複製 AI 指令」，也可下載圖片。支援 Web Share 的手機可按「分享圖片＋指令」，但實際能否同時帶入圖片與文字由手機系統及接收 App 決定。</p><p>內建指令要求忽略手寫內容，把遮住或不確定的地方標示為 <code>??</code>，不猜測且不先解題；最後詢問是否需要解題並提供觀念思考與速解步驟。把辨識結果貼回文字欄位後，仍需自行核對原圖。</p></section>
<section class="panel"><h2>6. 練習、熟練度與統計</h2><p>在「開始練習」選擇分類、出題範圍及 1–200 題。先自行作答，再翻開答案並判定答對或答錯；每題判定會立即保存。</p><p>預設連續答對 3 次後進入間隔複習，間隔依序為 1、3、7、14、30 天，之後每 30 天複習。任何階段答錯都會重置；熟練門檻可在設定調整，統計頁會顯示近期答題與各分類進度。</p></section>
<section class="panel"><h2>7. 儲存、備份與重置</h2><p>題庫、照片與進度只保存在目前網址及瀏覽器，沒有自動雲端同步；本機預覽和 GitHub Pages 的資料彼此獨立。換手機、瀏覽器或網址前，請先在「設定與資料」匯出 ZIP，再到新環境匯入。</p><p>匯入備份會取代目前題庫。「重置資料庫」會清除目前瀏覽器中的所有題目、照片、作答紀錄與熟練進度，並恢復預設分類；需要保留資料時請先匯出備份。</p></section>`;}
