...
<section class="panel"><h2>1. 快速新增題目</h2><ol><li>在「我的題庫」按「快速新增」，選擇科目，可直接輸入或選擇既有章節。</li><li>拖曳框選一道題目；可移動方框、拉動四邊或保留全圖。</li><li>答案選擇「使用題目原圖」、「另拍答案」、「選擇答案圖片」、「文字答案」或「稍後補答案」。</li><li>按「儲存並結束」，或按「儲存並繼續」立即加入下一題；連續新增時也可按「存檔結束」。</li></ol><p>每次按下儲存後，卡片就會立即寫入題庫。之後按取消只會放棄目前尚未儲存的題目，不會刪除前面已完成的卡片。</p><p>快速模式會記住上一次科目與章節、自動產生題目名稱，並以平衡畫質儲存。</p></section>
